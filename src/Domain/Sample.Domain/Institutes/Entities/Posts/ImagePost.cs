...
namespace Sofa.CourseManagement.Domain.Institutes.Entities.Posts
{
	public class ImagePost : PostBase
	{
		...
		public static ImagePost CreateInstance(Guid id, string title, short order, string content, Guid LessonPlanId)
		{
			...
			post.AddDomainEvent(new AddImagePostDomainEvent(post.Id, post.Title.Value, post.Order.Value, post.Content.Value, post.ContentType.Value, post.LessonPlanId));
		...
		public override void Update(string title, string content, ContentTypeEnum contentType, short order)
		{
			...
			AddDomainEvent(new UpdateImagePostDomainEvent(Id, Title.Value, Order.Value, Content.Value, ContentType.Value, LessonPlanId));
		}
	}
}
